...
if __name__ == "__main__":
    ...
    parser.add_argument("--stage-1-only", action="store_true", help="Force debug")
    parser.add_argument("--stage-2-only", action="store_true", help="Force debug")
    ...
    for stage_i in range(args.start_stage,4):
        ...
        if not args.debug:
            command = f"bash scripts/train.sh {config_path} {num_gpus} {accelerate_config_file}"
        else:
            command = f"python train.py --debug -c {config_path}"
